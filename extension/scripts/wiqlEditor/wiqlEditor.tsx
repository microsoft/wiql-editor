  ...
  ReactDom.render(
    <div className="header">
      <span className="bowtie">
        ...
        <button onClick={() => $(".wiq-input").click()} style={styles}>
          Import
        </button>
        <button className="wiq-export" style={styles}>
          Export
        </button>
        {isPanel && (
          <button
            onClick={() => $("#save").click()}
            id="saveQueryBtn"
            className="saveQueryBtn"
            style={saveQueryBtmstyles}
          >
            Save query
          </button>
        )}
        {!isPanel ? (
          <button className="open-in-queries" hidden style={styles}>
            {" "}
            Open in queries{" "}
          </button>
        ) : null}
      ...
      <span className="links">
        <a
          style={linkStyle}
          href="https://marketplace.visualstudio.com/items?itemName=ms-devlabs.wiql-editor"
          target="_blank"
        >
          Review
        </a>
        ...
        <a
          style={linkStyle}
          href="https://github.com/microsoft/wiql-editor/issues"
          target="_blank"
        >
          Report an issue
        </a>
...
export function setupEditor(
  ...
  renderToolbar(isPanel, async () => {
    ...
    const navigationService = (await VSS.getService(
      CommonServiceIds.HostNavigationService
    )) as IHostNavigationService;
    $(".open-in-queries")
      .show()
      .click(() => {
        ...
        const url = `${baseUrl}${
          project.id
        }/_queries/query/?wiql=${encodeURIComponent(wiql)}`;
